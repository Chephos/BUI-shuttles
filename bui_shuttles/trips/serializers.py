    ...
    def validate_route(self, value):
        ...
        if value.id not in driver_routes:
            raise serializers.ValidationError(
                "You can only create trips for your routes."
            )
        return value
...
class TripUpdate(serializers.ModelSerializer):
    ...
    def validate_take_off_time(self, value):
        ...
        if trip and trip.status != choices.TripStatus.not_started and value is not None:
            raise serializers.ValidationError(
                f"You cannot change the take off time after the trip has {trip.status}."
            )
        return value
